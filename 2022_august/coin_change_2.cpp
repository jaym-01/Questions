...
public:
    int change(int amount, vector<int> &coins)
    {
        ...
        {
            vector<vector<int>> mem = vector<vector<int>>(coins.size(), vector<int>(amount + 1, -1));
            return combinations(amount, coins, 0, mem);
        }
    }

    int combinations(int amount, vector<int> &coins, int coin, vector<vector<int>> &mem)
    {
        if (amount == 0)
        {
            return 1;
        ...
        // else if(coins[coin] > amount){
        //     return mem[coin][amount] = combinations(amount, coins, coin + 1, mem);
        // }
        else if(mem[coin][amount] != -1){
            return mem[coin][amount];
        }
        else
        {
            int left_over, tmp, total = 0;

            for (int i = coin; i < coins.size(); i++)
            {
                
                left_over = amount - coins[i];
                
                tmp = combinations(left_over, coins, i, mem);
